...
import { unpublishCourse } from "@/server/modules/courses/courses.service";
...
export async function POST(request: Request, { params }: { params: Promise<{ courseId: string }> }) {
  return executeRoute(request, async (context) => {
    ...
    const course = await unpublishCourse(owner.id, parsePathParam(courseIdParamSchema, courseId), context.requestId);
