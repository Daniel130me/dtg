...
| `grossRevenueMinor` | `sum(Payment.amountMinor where status = SUCCEEDED)` | Per currency; the dashboard reports the **primary currency** (highest-volume currency, `NGN` at launch) and other currencies are excluded from the headline (see `byCurrency` below). Legacy `USD` payment rows are normalized into `NGN` before counting (`normalizePlatformCurrency` in `analytics.logic.ts`) — the platform switched its launch currency to NGN, and inventing conversion rates is banned, so old USD rows are reported at face value inside NGN. |
